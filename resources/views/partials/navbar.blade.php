<nav id="navbar" class="fixed z-50 w-full md:w-0 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700" style="transition: top 0.3s;">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="md:hidden items-center">
                <a href="{{ url('') }}" class="flex ml-2">
                    ...
                    <span class="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap dark:text-white">Instantgram</span>
                </a>
            </div>
...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-52 xl:w-64 h-screen transition-transform -translate-x-full md:translate-x-0 bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
    <div class="h-full flex flex-col justify-between px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        <a href="{{ url('') }}" class="hidden md:flex ml-2 md:mr-24 pt-5">
            <img src="{{ URL::asset('assets/imgs/logo-exported.png') }}" class="h-8 mr-3" alt="Instantgram Logo" />
            <span id="instantgram_font" class="self-center text-xl xl:text-2xl sm:text-lg font-semibold whitespace-nowrap dark:text-white">Instantgram</span>
        </a>
        <ul class="space-y-2 font-medium h-fit flex flex-col items-stretch">
            ...
            <li>
                <a href="{{ route('search') }}" {{-- fix bang kalo mau --}} class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
            ...
            <li>
                <button data-modal-target="static-modal" data-modal-toggle="static-modal" class="flex w-full items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group" type="button">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
        ...
        <ul class="space-y-5 font-medium h-fit flex flex-col items-stretch pb-2">
            <li>
                <a href="{{ auth()->check() ? route('profileDetail', ['username' => '@' . auth()->user()->username]) : '' }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                    <div class="cover-images">
                        <img src="{{ asset('assets/imgs/profiles/' . auth()->user()->images) }}" alt="" srcset="" class="cover-image w-10 h-10 rounded-full">
                    </div>
                    <span class="flex-1 ml-3 whitespace-nowrap">{{ optional(auth()->user())->username }}</span>
                </a>
            </li>
            <li>
                <a href="{{ route('logout') }}" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group" wire-navigate>
